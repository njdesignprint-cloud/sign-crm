      ...
      $("jobsBody").innerHTML = rows.map(job => {
        ...
        return `
          <tr>
            ...
            ${showCommission ? `<td><div><strong>${safe(commission.salespersonName === "Unknown salesperson" ? (english ? "Unknown salesperson" : "Vendedor desconocido") : (commission.salespersonName || (english ? "Not assigned" : "Sin asignar")))}</strong> · ${commission.rate.toFixed(2)}%</div><div class="section-note">${english ? "Projected" : "Proyectado"}: ${money(commission.projected)}</div><div class="section-note">${english ? "Earned" : "Ganado"}: ${money(commission.earned)}</div><div><strong>${english ? "Outstanding" : "Pendiente"}: ${money(commission.available)}</strong></div>${commission.salespersonId && commission.rate <= 0 ? `<div class="pill st-cancelado mt-10">${english ? "Rate is 0%" : "La tasa es 0%"}</div>` : ""}${client?.salesSource === "salesperson" && client?.salespersonId && !commission.salespersonId ? `<div class="pill st-diseno mt-10">${english ? "Assignment missing" : "Falta asignación"}</div>` : ""}</td>` : ""}
            <td>
